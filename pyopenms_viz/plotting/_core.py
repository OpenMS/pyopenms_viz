...
class PlotAccessor:
    ...
    _msdata_kinds = ('chromatogram', 'spectrum')
    ...
    def _get_call_args(backend_name: str, data: DataFrame, args, kwargs):
        ...
        if isinstance(data, ABCDataFrame):
            ...
            raise ValueError(f"Unsupported data type: {type(data).__name__}, expected DataFrame.")
...
def _load_backend(backend: str) -> types.ModuleType:
    ...
    if backend == "bokeh":
        try:
            ...
            raise ImportError("Bokeh is required for plotting when the 'bokeh' backend is selected.") from None
        return module
        ...
        try:
            ...
            raise ImportError("Matplotlib is required for plotting when the 'matplotlib' backend is selected.") from None
        return module
        ...
        try:
            ...
            raise ImportError("Plotly is required for plotting when the 'plotly' backend is selected.") from None
        return module

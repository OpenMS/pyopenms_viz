...
def setup(app):
    ...
    shutil.copy(
        gallery_scripts_template / "GALLERY_HEADER.rst",
        gallery_scripts / "GALLERY_HEADER.rst",
    )
    ...
    for backend in [Path(i) for i in ["ms_matplotlib", "ms_bokeh", "ms_plotly"]]:
        ...
        shutil.copy(
            gallery_scripts_template / f"GALLERY_HEADER_{backend.name}.rst",
            gallery_scripts / backend / Path("GALLERY_HEADER.rst"),
        )

        # get valid files (filter out special files)
        files = [
            f
            for f in gallery_scripts_template.iterdir()
            if (f.suffix == ".py")
            and f.is_file()
        ...
        for file_name in files:
            ...
            with open(
                gallery_scripts / backend / f"{file_name.stem}_{backend.name}.py", "w"
            ) as new_file:
